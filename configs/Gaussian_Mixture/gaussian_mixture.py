...
def get_config():
    ...
    config.training.workspace='output/gaussian_mixture_100000'
    ...
    config.data.vector_dim_inference=100 #this is to hold the samples
    ...
    config.training.max_patience=10

    config.model.name = 'nlpe_rqs_posterior'
    config.model.nn_depth_bnaf = 3
    config.model.activation='tanh'
    config.model.flow_dimension=100               #T/F
    config.model.nn_block_dim=8
    config.data.summary_dim=0
    config.model.cond_dim=100


    
    #synthetic data parameters
    config.data.dataset = 'gaussian_mixture'
    ...
    config.data.vector_dim=config.model.flow_dimension
    ...
    config.model.correction_type='diagonal_neural'  # 'simple', 'diagonal_neural', or 'neural'
    ...
    config.model.lambda_shrinkage=0.00  # Stronger shrinkage prior toward delta function (no misspecification)
    ...
    config.training.use_initialization=True  # Enable initialization for better starting point
