...
"""
NNPE (Noisy Neural Posterior Estimation) configuration for SIR task
...
def get_config():
    ...
    config.training.workspace='output/nnpe_SIR_100000'
    ...
    config.data.vector_dim_inference=6 #this is to hold the samples
    config.sampling.sampling_name='nnpe'
    config.sampling.inference_method='SIR_task'
    ...
    config.model.name = 'noisy_neural_posterior'  # Use NNPE model with spike-and-slab corruption
    config.model.nn_depth_bnaf = 5
    config.model.activation='tanh'
    config.model.flow_dimension=2  # SIR has 2 parameters: β (infection rate) and γ (recovery rate)
    config.model.nn_block_dim=52
    config.data.summary_dim=0
    config.model.cond_dim=6  # SIR timeseries dimension

    
    #synthetic data parameters
    config.data.dataset = 'SIR'
    ...
    config.model.correction_type='hybrid'  # 'simple', 'diagonal_neural', or 'neural'
