...
"""
SIR Task Base Configuration
Serves as base config for all SIR experiments (RVNP-NN, RVNP-simple, wellspec)
Inherits from default_flow.py and defines SIR-specific parameters
"""
...
def get_config():
    ...
    config.data.vector_dim = 6
    config.data.vector_dim_inference = 6
    config.model.flow_dimension = 2
    config.model.cond_dim = 6
    ...
    config.data.julia_env_path = "."  # SIR uses Julia
    ...
    config.training.train_embedding_first = False  # SIR doesn't use embeddings
