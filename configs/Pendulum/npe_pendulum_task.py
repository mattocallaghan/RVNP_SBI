...
"""
NPE (Neural Posterior Estimation) configuration for Pendulum task
Uses standard neural posterior estimation with maximum likelihood training - no corruption or complex staging
Includes embedding support for high-dimensional timeseries data
"""
...
def get_config():
    ...
    # constants
    #################################
    config.training.workspace='output/npe_Pendulum_100000'
    ...
    config.model.embedding='IM' #InfoMax embedding for high-dimensional data
    config.model.hidden_scale=1
    config.model.embedding_dim=5  # Embedding dimension for pendulum timeseries
    config.model.flow_dimension=2  # Dimension of θ (pendulum parameters) - what we're learning posterior for
    ...
    config.data.vector_dim=2  # Low-dimensional after embedding
    ...
    # Enable embeddings for high-dimensional pendulum data
    config.model.train_embeddings=True       # NPE can use embeddings for high-dimensional data
    config.training.train_embedding_first=False  # Embeddings trained as part of flow training
